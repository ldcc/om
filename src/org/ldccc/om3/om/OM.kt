...
open class OM<O : PO> protected constructor(val clazz: Class<O>, val base: String) {
	...
	inline fun select(where: Select.() -> String): Select {
		return Select("SELECT * FROM $base WHERE").apply { "$sql ${where(this)}" }
	}
	...
	inner class Select(var sql: String) {
		...
		fun limit(limit: Int, offset: Int = 0): Select {
			return apply { "$sql LIMIT $limit OFFSET $offset" }
		}

		fun orderBy(vararg by: String): Select {
			return apply { "$sql ORDER BY ${by.joinToString()}" }
		}

		fun execute() = select(sql)
	}
